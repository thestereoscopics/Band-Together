...
export async function getBand(band: string) {
  try {
    const secret = process.env.DISCOGS_SECRET;
    const key = process.env.DISCOGS_KEY;
    const res = await fetch(
      `https://api.discogs.com/database/search?q=${band}&artist&key=${key}&secret=${secret}`
    );
...
export async function setPagination(getPage: string) {
  try {
